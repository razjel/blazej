...
package com.junkbyte.console.core 
{
	...
	public class ConsoleTools extends ConsoleCore{
		...
		public function map(base:DisplayObjectContainer, maxstep:uint = 0, ch:String = null, props:Array = null):void{
			if(!base){
		...
		public function mape(base:DisplayObjectContainer, maxstep:uint = 0, ch:String = null, props:Array = null):void{
			try {
				if(!base){
					report("Not a DisplayObjectContainer.", 10, true, ch);
					return;
				...
				while(index<list.length){
					...
					if(mcDO is DisplayObjectContainer){
						var mc:DisplayObjectContainer = mcDO as DisplayObjectContainer;
						try {
							var numC:int = mc['numElements'];
							for(var i:int = 0;i<numC;i++){
								var child:DisplayObject = mc['getElementAt'](i);
								list.splice(index+i,0,child);
							}
						} catch(e:Error) {
							var numC:int = mc.numChildren;
				...
				report(base.name + ":" + console.refs.makeRefTyped(base) + " has " + (list.length - 1) + " elements/sub-elements.", 9, true, ch);
				if (config.commandLineAllowed) report("Click on the child display's name to set scope.", -2, true, ch);
			} catch(e:Error) {
			}
		}
